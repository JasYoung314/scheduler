...
_sage_const_40 = Integer(40); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_7 = Integer(7); _sage_const_20 = Integer(20)
import os
import csv
import glob
import random
...
def schedule():
    ...
    for fle in files:
        ...
        for i in t2:
            p.add_constraint(sum( M[(tdict[i],j)]*w[(tdict[i],j)] for j in range(slots)) <=_sage_const_7  )

        for j in range(slots):
            p.add_constraint(sum( w[(i,j)] for i in range(Tutors)) <= _sage_const_2 )
        for j in range(slots):
            p.add_constraint(sum( w[(i,j)] for i in range(Tutors)) >= _sage_const_1 )
        for i in tutornames:
            for k in sk:
...
#        print e, schedule[e]
        ...
        print p.solve()<=_sage_const_40 
        return p.solve()<=_sage_const_40 
